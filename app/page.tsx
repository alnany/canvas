...
export default function Home() {
  ...
  return (
    <div style={{ background:"#0a0a0f", color:"#e2e8f0", fontFamily:"'Share Tech Mono','Courier New',monospace", minHeight:"100vh" }}>
      ...
      {/* NAV */}
      <nav style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:`14px ${px}`, borderBottom:"1px solid #1e1e3f", position:"sticky", top:0, zIndex:50, background:"rgba(10,10,15,0.95)", backdropFilter:"blur(12px)" }}>
        <span style={{ color:"#a855f7", fontFamily:"'Press Start 2P',monospace", fontSize: isMobile ? 11 : 13, letterSpacing:2 }}>CANVAS</span>
        <div style={{ display:"flex", gap: isMobile ? 10 : 24, alignItems:"center" }}>
          {!isMobile && <a href="#mechanics" style={{ color:"#94a3b8", textDecoration:"none", fontSize:13 }}>Mechanics</a>}
          {!isMobile && <a href="#tokenomics" style={{ color:"#94a3b8", textDecoration:"none", fontSize:13 }}>Tokenomics</a>}
          <Link href="/play" style={{ padding: isMobile ? "7px 14px" : "8px 20px", background:"#7c3aed", color:"#fff", borderRadius:6, textDecoration:"none", fontWeight:"bold", fontSize: isMobile ? 11 : 13 }}>
            Play Demo
          </Link>
        </div>
      </nav>
      ...
      {/* HERO */}
      <section style={{ display:"flex", flexDirection: isMobile ? "column" : "row", alignItems:"center", justifyContent:"space-between", maxWidth:1100, margin:"0 auto", padding: isMobile ? "40px 20px 32px" : "80px 32px", gap: isMobile ? 32 : 48 }}>
        <div style={{ flex:1, textAlign: isMobile ? "center" : "left" }}>
          ...
          <h1 style={{ fontFamily:"'Press Start 2P',monospace", fontSize: isMobile ? "clamp(26px,8vw,40px)" : "clamp(32px,5vw,60px)", color:"#a855f7", margin:"0 0 16px", lineHeight:1.3 }}>
            CANVAS
          </h1>
          <p style={{ fontSize: isMobile ? 16 : 18, color:"#cbd5e1", marginBottom:8, maxWidth:480, lineHeight:1.6, ...(isMobile ? { margin:"0 auto 8px" } : {}) }}>
            A <span style={{ color:"#a855f7" }}>pixel war game</span> on Solana.<br />
            Claim territory. Earn tokens. Survive the chaos.
          </p>
          <p style={{ fontSize:14, color:"#94a3b8", maxWidth:420, lineHeight:1.7, ...(isMobile ? { margin:"8px auto 28px" } : { marginBottom:32 }) }}>
            1,000 × 1,000 grid. Real-time PVP. Every pixel has a price.
          </p>
          <div style={{ display:"flex", gap:12, justifyContent: isMobile ? "center" : "flex-start", marginTop: isMobile ? 0 : 4 }}>
            <Link href="/play" style={{
              padding: isMobile ? "11px 20px" : "14px 28px", background:"linear-gradient(135deg,#7c3aed,#a855f7)",
              color:"#fff", borderRadius:8, textDecoration:"none", fontWeight:"bold", fontSize: isMobile ? 12 : 14,
              boxShadow:"0 0 24px rgba(168,85,247,0.45)"
            }}>Try the Demo</Link>
            <a href="#mechanics" style={{
              padding: isMobile ? "11px 16px" : "14px 28px", border:"1px solid #4c1d95",
              color:"#a855f7", borderRadius:8, textDecoration:"none", fontSize: isMobile ? 12 : 14
            }}>How it works</a>
          </div>
          <div style={{ marginTop:24, fontSize:12, color:"#475569", display:"flex", alignItems:"center", gap:8, justifyContent: isMobile ? "center" : "flex-start" }}>
      ...
      {/* CORE LOOP */}
      <section id="mechanics" style={{ maxWidth:1100, margin:"0 auto", padding: isMobile ? "40px 16px" : "60px 32px" }}>
        <div style={{ textAlign:"center", marginBottom: isMobile ? 32 : 48 }}>
          <div style={{ fontSize:11, color:"#a855f7", letterSpacing:3, textTransform:"uppercase", marginBottom:8 }}>Core Loop</div>
          <div style={{ fontFamily:"'Press Start 2P',monospace", fontSize: isMobile ? 14 : 18 }}>How it works</div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: isMobile ? 12 : 16 }}>
          {STEPS.map((s,i) => (
            <div key={i} style={{ padding: isMobile ? 16 : 24, background:"#12121a", border:"1px solid #1e1e3f", borderRadius:12 }}>
              <div style={{ fontSize: isMobile ? 22 : 28, marginBottom:10 }}>{s.emoji}</div>
              <div style={{ color:"#c084fc", fontWeight:"bold", marginBottom:8, fontSize: isMobile ? 11 : 13 }}>{i+1}. {s.label}</div>
              <p style={{ color:"#94a3b8", fontSize: isMobile ? 11 : 13, lineHeight:1.7, margin:0 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* STRIKES */}
      <section style={{ maxWidth:1100, margin:"0 auto", padding: isMobile ? "0 16px 40px" : "0 32px 60px" }}>
        <div style={{ background:"#0d1117", border:"1px solid #0e3a4f", borderRadius:16, padding: isMobile ? 20 : 32 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap: isMobile ? 10 : 16 }}>
            {[
              ...
              <div key={t.tier} style={{ padding: isMobile ? 12 : 20, borderRadius:12, textAlign:"center",
                border:`1px solid ${t.color}50`, boxShadow:`0 0 16px ${t.glow}` }}>
                <div style={{ fontSize: isMobile ? 9 : 11, color:t.color, marginBottom:4 }}>{t.tier}</div>
                <div style={{ fontSize: isMobile ? 22 : 32, fontWeight:"bold", color:t.color }}>{t.mult}</div>
                <div style={{ fontSize: isMobile ? 9 : 11, color:"#475569", marginTop:4 }}>{t.chance} chance</div>
              </div>
            ))}
          </div>
      ...
      {/* TOKENOMICS */}
      <section id="tokenomics" style={{ maxWidth:1100, margin:"0 auto", padding: isMobile ? "0 16px 40px" : "0 32px 60px" }}>
        <div style={{ textAlign:"center", marginBottom: isMobile ? 32 : 48 }}>
          <div style={{ fontSize:11, color:"#a855f7", letterSpacing:3, textTransform:"uppercase", marginBottom:8 }}>Tokenomics</div>
          <div style={{ fontFamily:"'Press Start 2P',monospace", fontSize: isMobile ? 14 : 18 }}>$CANVAS - 10B Supply</div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns: isMobile ? "repeat(1,1fr)" : "repeat(3,1fr)", gap:16, marginBottom:16 }}>
          {TOKENOMICS.map(t => (
            <div key={t.label} style={{ padding: isMobile ? 16 : 24, background:"#12121a", border:`1px solid ${t.color}30`, borderRadius:12 }}>
              <div style={{ fontSize: isMobile ? 28 : 36, fontWeight:"bold", color:t.color, marginBottom:4 }}>{t.pct}%</div>
      ...
      {/* HOLDR */}
      <section style={{ maxWidth:1100, margin:"0 auto", padding: isMobile ? "0 16px 40px" : "0 32px 60px" }}>
        <div style={{ background:"linear-gradient(135deg,#1a0533,#0f172a)", border:"1px solid #4c1d95", borderRadius:16, padding: isMobile ? 20 : 32, display:"flex", gap:20, alignItems:"flex-start" }}>
          <div style={{ fontSize: isMobile ? 32 : 40, flexShrink:0 }}>💎</div>
          <div style={{ flex:1, minWidth:0 }}>
            <h3 style={{ color:"#c084fc", fontWeight:"bold", fontSize: isMobile ? 15 : 18, marginBottom:8 }}>Holdr Class</h3>
      ...
      {/* CTA */}
      <section style={{ textAlign:"center", padding: isMobile ? "40px 20px 56px" : "60px 32px 80px" }}>
        <div style={{ fontFamily:"'Press Start 2P',monospace", fontSize: isMobile ? "clamp(13px,4vw,20px)" : "clamp(18px,3vw,28px)", marginBottom:16, lineHeight:1.6 }}>
        ...
        <Link href="/play" style={{
          display:"inline-block", padding: isMobile ? "14px 32px" : "16px 48px", fontFamily:"'Press Start 2P',monospace", fontSize: isMobile ? 11 : 12,
      ...
      {/* FOOTER */}
      <footer style={{ borderTop:"1px solid #1e1e3f", padding:`24px ${px}`, textAlign:"center", fontSize:11, color:"#475569" }}>
